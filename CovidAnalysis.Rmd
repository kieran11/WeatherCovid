...
CheckMainParamsMnMdl = dget("Data/PosteriorDistr") %>%
...
RemoveOutliersMn = CheckMainParamsMnMdl %>% 
  group_by(Var) %>% 
  summarise(Lowest = quantile(Value, .025),
            Highest = quantile(Value, .975)) %>% 
  ungroup()

MainParamsPltMn = CheckMainParamsMnMdl %>% 
  inner_join(RemoveOutliersMn, by = "Var") %>% 
...
CheckMainParamsAllMdl = dget("Data/PosteriorDistr") %>%
  select(b_lag_temperatureMax_06, b_lag_temp_sqr,  `b_lag_temperatureMax_06:lag_precipIntensity_06`,
         b_lag_precipIntensity_06, Model ) %>% 
  tidyr::gather(Var, Value, -Model) 

RemoveOutliersAll = CheckMainParamsAllMdl %>% 
  group_by(Var, Model) %>% 
  summarise(Lowest = quantile(Value, .025),
            Highest = quantile(Value, .975)) %>% 
  ungroup()

MainParamsPltAll = CheckMainParamsAllMdl %>% 
  inner_join(RemoveOutliersAll, by = c("Var", "Model")) %>% 
  filter(Value <= Highest & Value >= Lowest) %>% 
  ggplot(., aes(x = Value, fill = "blue")) +
  geom_density() + 
  theme_classic() +
  geom_hline(yintercept = 0, colour = "white") +
  facet_wrap(~Var + Model, scales = "free_y") +
  theme(legend.position = "bottom") +
  labs(x = "Posterior Parameter Distribution")
...
CovidMEBest = dget("Data/ConditionalEffects") %>% 
  filter(Model == "CovidBayesianStdCensus" ) %>% 
  select(lag_temperatureMax_06 , lag_precipIntensity_06, estimate__, 
         lower__,  upper__) %>% 
  mutate(lag_precipIntensity_06= factor(lag_precipIntensity_06, levels = c("-1", "0", "1") )) %>% 
  ...
  geom_ribbon(aes(ymin=lower__ , ymax=upper__), alpha=0.2) +
  theme_classic() +
  facet_wrap(~ lag_precipIntensity_06 , ncol = 3) +
  labs(y = "Estimated New Cases", x= "Lagged Max Temperature")
